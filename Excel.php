<?php
   ...
   *    'mode' => 'import',
   *    'fileName' => $fileName,
   *    'setFirstRecordAsKeys' => true, // if you want to set the keys of record column with first record, if it not set, the header with use the alphabet
   *    column on excel.
   *    'setIndexSheetByName' => true, // set this if your excel data with multiple worksheet, the index of array will be set with the sheet name. If this not
   *    set, the index will use numeric.
   ...
   * @property string|array    $fileName             is a name for file name to export or import. Multiple file name only use for import mode, not work if you
   *           use the export mode.
  ...
  class Excel extends \yii\base\Widget
  {
    ...
    public $formatter;
    /**
     * @var use auto size for column width.
     */
    public $autoSizeColumn = false;
      ...
      foreach ($models as $model)
      {
        ...
        if ($this->setFirstTitle && !$hasHeader)
        {
          ...
          foreach ($columns as $key => $column)
          {
            ...
            $activeSheet->getColumnDimension($col)->setAutoSize($this->autoSizeColumn);
